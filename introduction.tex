...
\begin{equation}
	eT_\text{e}=m_\text{e} v_{\text{T}_\text{e}}^{^2}
\end{equation}
...
\begin{equation}
	v_{\text{T}_\text{i}}\approx\left(m_\text{e}/m_\text{i}\right)^{^\text{\textonehalf}}v_{\text{T}_\text{e}}
\end{equation}

La figure (\ref{zoologie}), issue du livre du National Research
Council \cite{national1995Plasma}, représente une classification des plasmas
...
\begin{equation}
\label{1-paramPlasma}
\Gamma=\frac{<E_\text{p}>}{<E_\text{c}>}=\frac{e^{^2}n_\text{e}^{^{1/3}}}{\varepsilon_{_0}
eT_\text{e}}
\end{equation}
...
\begin{equation}
\mathbf F=q_\alpha(\mathbf E+ \mathbf v_\alpha\times\mathbf B)
\end{equation}

l'indice $\alpha$ dénotant le type de la particule chargée, $q_\alpha$ sa
charge et $(E,B)$ le champ électromagnétique. Elle se décompose
principalement sur deux échelles de temps :
...
\begin{equation}
\omega_\text{p}\left[\left(\frac{n_{_0}e^{^2}}{\varepsilon_{_0}
m_\text{e}}\right)^\text{\textonehalf}
\right]\cdot\lambda_D\left[\left(\frac{\varepsilon_{_0}
eT_\text{e}}{n_{_0}e^{^2}}\right)^\text{\textonehalf}\right]
...
\begin{equation}
	\nu_\text{ei}\approx\frac{4\sqrt{2\pi}e^{^4} n_\text{e} \ln\Lambda
	}{3(4\pi\varepsilon_{_0})^{^2}m_\text{e}^{^{1/2}}T_\text{e}^{^{3/2}}}
\end{equation}

avec $\ln \Lambda$ le logarithme de Coulomb,
$\Lambda=\lambda_\text{D}(e^{^2}/4\pi\varepsilon_{_0}T_\text{e})^{\moinsun}$
étant le ratio entre la longueur de Debye et une distance minimale d'approche. 
...
représente alors la distance que peut parcourir une particule avant de subir
une certaine interaction avec le système qui l'entoure :
...
chaque collision, et entraîne un déplacement macroscopique moyen non nul, décrit
par le coefficient de mobilité :
...
le transport des particules. Les plasmas magnétisés sont de plus fortement
...
\begin{itemize}
  ...
  \item Un mouvement cyclotronique, rotation rapide de la particule
  autour d'un centre-guide dans le plan perpendiculaire aux lignes de champ
  \item Un mouvement de dérive, lent devant le mouvement cyclotronique, dû à la
  modification de la trajectoire du centre guide en réponse aux forces
  appliquées
\end{itemize}
...
\subsubsection{De la statistique au fluide}
