...
\nabla\cdot\left(\varepsilon\mathbf{u}_\alpha+\mathbf
P_\alpha\cdot\mathbf{u}_\alpha + \mathbf q_\alpha\right) =q_\alpha
n_\alpha\mathbf E_\alpha\cdot
\mathbf{u}_\alpha+\mathbf{R_\alpha}\mathbf{u}_\alpha+Q_\alpha
\end{equation}

où $\varepsilon=m_\alpha n_\alpha u_\alpha^2/2+3p_\alpha/2$ représente la
densité d'énergie totale du fluide et 
...
repère au repos du fluide. Enfin, $Q_\alpha$ est la production totale de
chaleur lors de collisions et de réactions chimiques. Pour les électrons, cette
puissance collisionnelle est généralement négative et peut s'exprimer en terme de taux:
\begin{equation}
	Q_e=-en_e\sum_{i}\left(U_in_ik_i+\frac{2m_e}{m_i}k^m_i\varepsilon_e\right)
\end{equation}
