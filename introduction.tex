...
\begin{equation}
	eT_\text{e}=m_\text{e} v_{\text{T}\text{e}}\puissance{2}
\end{equation}
...
\begin{equation}
	v_{\text{T}\text{i}}\sim\left(m_\text{e}/m_\text{i}\right)^{\text{\textonehalf}}
	v_{\text{T}\text{e}}
\end{equation}
...
=v_{\text{T}\text{e}}\left[\left(\frac{eT_\text{e}^{^{\phantom{1}}}}{m_\text{e}}\right)^{\text{\textonehalf}}\right]
\end{equation}
...
\end{equation}
 ...
 est de moyenne nulle sur un temp $t\sim\omega^{-1}$, la projection
...
L'influence de ce courant est fondamental dans la stabilité du plasma. En
se souvenant de l'hypothèse de quasineutralité, ce courant s'écrit :
...
Dans le cas d'un champ électrique dépendant du temps, une troisième dérive, d'un
ordre de grandeur inférieure aux dérives électrique et diamagnétique,
...
\begin{equation}
\label{1-vitessePol}
\mathbf{u}_p^\alpha= \frac{\text{d}\mathbf
u_E}{\text{dt}}=-\frac{m_\alpha}{q_\alpha B^2}\frac{\text{d}\nabla_\perp
U}{\text{dt}}
\end{equation}

C'est la dérive de polarisation $\mathbf u^\alpha_p$, qui dérive du terme
d'inertie. Bien qu'elle soit d'un ordre de grandeur inférieur aux deux autres
...
compte du terme collisionnel dans l'équation du mouvement est donc essentielle.
