...
trajectectoire, et ainsi se déplacer instantanément dans des régions éloignées de leur espace des
...
approximations\parencite{Approximation bi-terme, Approche homogène, approche
non-locale}. De plus, quand une espèce est suffisement collisionnelle, sa
distribution en vitesse relaxe vers une maxwellienne locale sur une échelle de
...
de Maxwelll, l'équation de Boltzmann s'intègre donc facilement dans l'espace des
vitesses, pour former les équations fluide de conservation. Le nombre de degrés
de liberté du problème est ainsi réduit en ne conservant de l'état du plasma
...
$\mathbf M^{(0)}$, $\mathbf M^{(1)}$ et $\mathbf M^{(2)}$ ont chacun un nom et
une interprétation physique simple. La densité $n$, qui
...
ce tenseur se décompose en introduisant la pression
scalaire $p_\alpha$ et le tenseur de viscosité $\boldsymbol{\Pi}_\alpha$:

\begin{equation}
\mathbf P_\alpha=p_\alpha\,\mathbf I + \boldsymbol{\Pi}_\alpha
\end{equation}

avec $\mathbf I$ la matrice  
Pour les distributions maxwelliennes, $T_\alpha=p_\alpha/n_\alpha$ représente la
température partielle mesurée en unité d'énergie :

\begin{equation}
p_\alpha=en_\alpha T_\alpha\
\end{equation}

 d'ordre trois (le flux de chaleur),
sont
...
Leur évolution est contrôlée par les équations fluides obtenues en intégrant
