...
\mathbf{u}_\alpha\mathbf{u}_\alpha+\mathbf{P_\alpha}\right)=\mathbf
F_\alpha n_\alpha+\mathbf{R_\alpha}
\end{equation}

avec $\mathbf{P_\alpha}$ le tenseur de pression, $\mathbf{F_\alpha}$
les forces extérieures et $\mathbf{R_\alpha}$ un terme de
...
+\left(\nu_\alpha+\nu_\text{iz}\right) \mathbf u_\alpha=\frac{\mathbf
F_\alpha}{m_\alpha} -\frac{\nabla p_\alpha}{m_\alpha n_\alpha}
...
par $m_\alpha {u}_\alpha^2/2$, s'écrit en ne considérent que la force de
Lorentz dont la force magnétique ne travaille pas :
...
où $\varepsilon_\alpha=m_\alpha n_\alpha u_\alpha^2/2+3p_\alpha/2$ représente
la densité d'énergie du fluide et 
	$$\mathbf q_\alpha=m_\alpha n_\alpha\left<w^2\mathbf w\right>/2$$
est le flux de chaleur (d'énergie chaotique), ie. le flux d'énergie dans le
repère au repos du fluide. Enfin, $Q_\alpha$ est la production
\emph{totale}\footnote{Dans de nombreux ouvrages, le terme de chauffage
...
chauffage par friction (ie. l'effet Joule) est donc toujours présent. C'est
d'ailleurs un terme primordial pour capturer correctement le comportement de la
température électronique.} de chaleur lors de collisions et de réactions chimiques.
...
\begin{equation}
	...
	v=-en_e\sum_{i}\left(U_in_ik_i+\frac{2m_e}{m_i}k^m_i\varepsilon_e\right)
\end{equation}

On peut maintenant soustraire la contribution de l'énergie dirigée de
...
\frac{3}{2}\nabla\cdot\left(p_\alpha
\mathbf{u}_\alpha\right)+p_\alpha\nabla\cdot\mathbf u_\alpha
+\boldsymbol{\Pi}_{\alpha j k}\frac{\partial u_{\alpha j}}{\partial{x_k}} +
\nabla\cdot\mathbf q_\alpha={Q_\alpha}
\end{equation}

Les changements dans l'énergie interne du fluide sont donc dus aux flux nets
d'énergie arrivant ou partant d'un point
\subsubsection{Conservation de la chaleur}

De plus, quand une espèce est
