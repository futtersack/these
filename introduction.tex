...
$\boldsymbol{\Pi}=0$. L'équation de mouvement \eqref{1-eqMouvement} se réduit
ainsi à un équilibre entre les forces d'inertie, de traînée, de Lorentz et de
pression :
...
électroniques et ioniques. En supposant une vitesse caractéristique de
l'écoulement $V\sim|\mathbf u|$ on construit un jeu de nombres adimensionnés :
\begin{itemize}
  \item Le nombre de Mach $M_a=V/v_T$ dérive du ratio entre le terme
  d'inertie (l'énergie cinétique) et le terme de pression (l'énergie interne).
  Il quantifie les effets de compressibilité du fluide et permet de distinguer
  les écoulements subsonique/supersoniques.
  \item Le nombre de Reynolds $R_e=mnVL/\nu$
  
  \end{itemize}Les termes d'inertie
dépendent de la longueur de gradient de la vitesse fluide et donc de la taille du plasma $L$. Le terme de collision, quant à lui, dépend du libre parcours moyen des particules
$\lambda=|u|/\nu$. Le terme de pression peut être dominant pour les espèces dont
la température est élevée Dans le cas de plasmas collisionnels,
fortement magnétisé, ou


\subsection{Modèle non magnétisé de dérive-diffusion}
\label{1-transportAmbipolaire}
\subsubsection{Plasmas de décharge collisionnels}
 industriels et de laboratoires
\subsubsection{Equations de dérive-diffusion}

Dans les plasmas basse-température, qui possèdent un faible degré d'ionisation,
la dynamique est dominée par la perte de quantité de mouvement
liée à l'ionisation et au frottement avec le gaz. En
l'absence de champ magnétique et pour des particules collisionnelles chargées,
l'équation du mouvement se simplifie alors en négligeant les termes d'inertie
relativement faibles\footnote{Les termes inertiels peuvent être négligés quand
la vitesse fluide $\mathbf u$ est subsonique : $\mathbf u$ vérifie alors
$\mathbf u\ll \left<v\right>/K_n$, où $K_n=\lambda/L$ est le nombre de Knudsen
et $$\nu \mathbf u=\frac{\left<v\right>\mathbf u}{\lambda}\gg
\frac{u^2}{\lambda}K_n=\frac{u^2}{L}\sim \mathbf u\cdot\nabla\mathbf u$$} par
rapport au terme collisionnel :

\begin{equation}
\label{1-eqBilanForce}
n\mathbf u=\frac{q}{\nu m}n\mathbf
E-\frac{\nabla\left(neT\right)}{\nu m}\equiv\pm\mu n\mathbf
E-{\nabla\left(Dn\right)}
\end{equation}

C'est l'équation bien connue de dérive-diffusion, avec les deux coefficients de
transport : 
\begin{itemize}
  \item la mobilité, $\mu=e/\nu m$, qui mesure la disposition d'une espèce à
  laisser passer le courant dans un milieu
  \item le coefficient de diffusion $D=eT/\nu m$ qui représente 
\end{itemize}
