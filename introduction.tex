...
représentant le nombre moyen de particules occupant un volume infintésimal
de l'espace des phases à un instant $t$. Son évolution est alors décrite à
travers l'équation de Boltzmann.

Sur des échelles de temps suffisement longues, l'évolution d'un plasma est 
essentiellement due à des phénomènes de transport macroscopiques pour lesquels
...
espèce d'une manière statistique, comme un continuum défini par une fonction de
...
$\vec\nabla$ étant l'opérateur gradient appliqué à l'espace des positions ou des
vitesses, et $\mathbf{F_\alpha}$ se résumant souvent aux forces
électromagnétiques. \eqref{1-BoltzmannEquation} est très
difficile à résoudre du fait de la complexité de l'opérateur de collisions $C[f_\alpha]$.
En effet, lors de collisions, les particules peuvent changer de trajectectoire,
et ainsi se déplacer instantanément dans des régions éloignées de leur espace des
...
ondes-particules ou celles implicant des particules rapides. 
Cependant, les méthodes utilisées pour la résoudre nécessitent de
nombreuses approximations\footnote{Approximation bi-terme, Approche
homogène, approche non-locale}. De plus, quand une espèce est suffisement
collisionnelle, sa distribution en vitesse relaxe rapidement vers une
maxwellienne locale :
...
Les intégrales d'une maxwellienne sont facilement calculables et favorisant
donc l'utilisation des équations fluides, la
description macroscopique donnant une interprétation plus simple des phénomènes entrants en jeu. Le nombre de degrés de liberté du problème est ainsi
...
distribution \eqrefp{1-MaxwellDistribution}, que le plasma est à l'équilibre
