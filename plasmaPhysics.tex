\Chapter{Introduction}
	\section{Fondamentaux de la physique des plasmas}
		...
		La dynamique du plasma résulte du couplage entre le mouvement des particules
		chargées et les forces de Coulombs ($\mathbf F=q_s\mathbf E$) et de Lorentz
		($\mathbf F=q_s\mathbf v_s\times\mathbf B$) présentent ou qui se forment dans
		le système.
			...
			\begin{wrapfigure}{r}{0.4\textwidth}
    			\includegraphics[width=0.40\textwidth]{figures/particleDrifts.png}
  				\caption{Mouvement cyclotronique et de dérive des particules dans un
  				champ magnétique.}\label{particleDrifts}
			\end{wrapfigure}
			La présence d'un champ magnétique va aussi fortement influencer le transport
			du plasma. Le mouvement d'une particule chargée dans un champ
			magnétique sujette à la force de Lorentz se décrit essentiellement par trois
	...
	\lipsum{50}
		...
		\section{Les gaines électrostatiques}
			Les gaines électrostatiques sont des structures non-neutres qui se développent aux frontières du plasma
			du fait de la plus grande vitesse des électrons .
			Ces structures se forment ont une taille de l'ordre de grandeur de la longueur de Debye $\lambda_D$ L'interaction dite plasma-paroi est une branche à part entière de la physique des plasmas.
			En effet, au contact d'un milieu extérieur, que ce soit un obstacle matériel ou un gaz ambiant, le plasma perd sa quasineutralité.
