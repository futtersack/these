\Chapter{Introduction}
	\section{Fondamentaux de la physique des plasmas}
		...
		chargées et les forces de Coulombs ($\mathbf F=q\mathbf E$) et de Lorentz
		($\mathbf F=-q\mathbf B\times\mathbf v$) présentent ou qui se forment dans le
		système.
			...
			du plasma. Une particule chargée en mouvement dans un champ magnétique
			est sujette à la force de Lorentz $\mathbf{F}=q_s\mathbf{v}\times\mathbf{B}$.
			Son mouvement se décrit alors par deux composantes :
			\itemize
			\item Un déplacement parallèle aux lignes de champ
