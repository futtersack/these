...
\end{figure}
 ...
 L'apparition de cette instabilité est directement reliée au gradient de
 température électronique et au terme d'inertie  :
 
 \begin{itemize}
   \item Un plasma isotherme s'étend dans le sens du flux d'électron
   transverse tandis que la variation de la longueur de gradient influe
   sur le profil de température et modifie l'orientation des ondes
   acoustiques. 
	\item Les solutions du modèle quand le terme inertiel d'advection est
	négligé sont elles aussi stationnaires.
\end{itemize}
...
figure~\ref{4-PegasesCarteDensiteVarBiasWave}. Entre les surdensités, il se crée
un champ électrique, qui combiné au filtre magnétique est responsable d'un
mouvement de dérive du type ExB autour des fronts exacerbé fortement le
transport transverse des électrons.


La caractérisation complète de cette instabilité est à réaliser, mais
s'avère d'ores et déjà difficile : bien que la vitesse des particules et des
fronts d'ondes ne .
On peut se poser la question de la réalité physique de phénomène.
