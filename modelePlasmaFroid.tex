...
{n_e\left(\mathcal{P}_\text{ext}-\Pi\right)}+(2n_e\nu_e+\mathcal{S}_e)\mathcal{U}_e
\end{equation}
...
dans le plasma et $\Pi$ le terme de perte d'énergie dûe aux collisions. Le chauffage
...
\nabla_\perp\cdot\left(\bar{n}_\alpha\bar{\mathbf{u}}_{\alpha_\perp}\right)=\mathcal{S}_\alpha
...
(\frac{5}{2}\mathcal{S}_e-\partial_t\bar{n}_e)\bar{T}_e =\\
&\bar{n}_e\bar{\mathbf{u}}_{e_\perp}\cdot(\nabla_\perp
\bar{\Phi}-\frac{5}{2}\nabla_\perp
\bar{T}_e)+\bar{n}_e(\mathcal{P}_\text{ext}-\Pi)-\mathcal{L}_{\varepsilon_\para}
...
Hagelaar~\parencite{Hagelaar}, nous utilisons cette technique pour quelques
termes du modèle.} :
\begin{equation*}
...
A_\varepsilon=A_e\frac{5e^2n_eT_e}{2m_e}\;\;\;\;\text{et}\;\;\;B_\varepsilon=B_e\frac{5e^2n_eT_e}{2m_e}
\end{equation*}

L'équation d'énergie est utilisée pour résoudre ce changement de température

\begin{equation}
\label{3-eqTemperature}
\left(\frac{3en_e}{2\Delta t}+\frac{5}{2}\mathcal S_ee\right)\mathcal T
+\nabla\cdot\left(A_\varepsilon\nabla\mathcal T+B_\varepsilon\mathbf b\times\nabla\mathcal T\right) + \frac{5}{2}\mathcal{S}_e
eT_e = eT_e\frac{\text{d}n_e}{\text{dt}}+ {n_e\left(\mathcal{P}_\text{ext}-\Pi\right)}+(2n_e\nu_e+\mathcal{S}_e)\mathcal{U}_e
\end{equation}

$\mathcal T=T_e^{k+1}-T_e^{k}$
\subsection{Etude de convergence en espace}
