...
mouvement et de celle du courant. Ecrivons \eqref{3-eqMouvement} comme une
équation algébrique d'inconnue $\mathbf u$, reliant la fréquence caractéristique du transport
...
L'expression ainsi obtenue pour la vitesse fluide fait intervenir le champ
électrique à travers le terme $\mathbf a$. 
\begin{equation}
\label{3-eqCorrection}
\mathbf a(\mathbf E^{k+1}) = \mathbf a(\mathbf E^{k})-\frac{q}{m}\nabla
(\Phi^{k+1}-\Phi^{k})
\end{equation}
Avec ces expressions, l'équation du courant \eqrefp{eqCourant} 
