...
équations est complexe voire impossible et le problème est alors
généralement adressé à travers le développement de modèles cinétiques numériquement très
...
sources effectifs. L'équation d'énergie tient compte des pertes associées à
l'ensemble des collisions (dont celles dues à l'ionisation des particules) et
contient un terme de puissance
absorbée avec un profil fixé. Enfin, une équation est dédiée au flux de chaleur
magnétisé.
...
avec $k^m_{\alpha
s}=\left<\sigma^m_{\alpha s}v_r\right>$, où $\sigma^m_{\alpha
s}$ est la section efficace de l'interaction et $v_r$ la vitesse relative des
particules en collision. Un grand
...
température électronique $T_e$ et qui, dans le cas d'un
plasma simple électrons-ions, se limite à l'ionisation : 

\begin{equation}
\mathcal S_e=\mathcal
S_i=n_en_gk^\text{iz}(T_e) 
\end{equation}
...
{en_e\left(\mathcal{P}_\text{ext}-Q_e\right)}+(2n_e\nu_e+\mathcal{S}_e)\mathcal{U}_e
\end{equation}

où $T_e$ est la température électronique exprimée en eV,
$\text{d/dt}=\partial_t+u_e\cdot\nabla$ représente la dérivée totale,
$\mathcal{S}_e$ est le terme source d'électrons lié à l'ionisation, $\mathcal{P}_\text{ext}$ la puissance extérieure déposée
dans le plasma et $Q_e$ le terme de perte d'énergie due aux collisions, tous
...
l'intermédiaire des collisions (voir
...
perte effective d'une espèce en entrée de gaine. Les flux d'ions positifs sont définis
par :
...
le critère de Bohm et nous laissons le modèle quasineutre résoudre la vitesse
...
Nous limitons cependant le facteur de Boltzmann afin que celui-ci ne dépasse pas
l'unité, ce qui permet de s'assurer que la gaine ne s'inverse pas. Le flux
d'électron s'ajuste alors pour réguler potentiel
autour d'une valeur d'équilibre, définissant une condition limite pour le
courant en fonction de la polarisation de la paroi :
...
Dans
le cas d'un plasma n'étant composé que d'une seule espèce d'ion, on peut définir
le potentiel flottant normalisé $\Lambda=({m_i}/{2\pi m_e})^{\text{\textonehalf}}$.
La condition \ref{3-sheathcurrent} peut se factoriser en :
...
m_e}{eT_e}}-\frac{1}{n_e}\sum_in_iW_i\right)
...
minimum atteindre la vitesse de Bohm \eqref{3-fluxIonique} pour permettre une
rupture de la quasineutralité du plasma et l'apparition d'une gaine de charge
d'espace positive.
...
équilibre de Boltzmann le long des lignes, permettent le transport rapide des
fluctuations dans la direction parallèle. Le problème du
...
nets de perte $\mathcal L$ (ou éventuellement de source pour le courant) dans
...
Après cette opération de réduction, le système d'équation peut se réécrire en
...
\begin{enumerate}
  ...
  électrostatique $\Phi$ à travers la conservation du courant, en s'appuyant sur
  la réponse des vitesses fluides au changement $\Delta \mathbf
...
Dans les équations, les opérateurs de divergence sont discrétisés en suivant une
méthode Volumes Finis~\parencite{toro}. Le domaine de simulation est
schématiquement représenté sur la figure~\ref{3-maillage}.

\begin{figure}[!htbp]
\centering
\includegraphics[width=0.6\textwidth]{figures/3-magnisGrid.png}
...
les directions $\mathbf{x}$ et $\mathbf{y}$. Les quantités scalaires sont
définies aux noeuds du maillage ainsi que sur les bords du domaine, et se
repèrent par les indices $(i,j)$. Les quantités
vectorielles sont définies à mi-chemin entre les noeuds, aux indices $(i+1/2,j)$
pour leur composante selon $x$ et $(i,j+1/2)$ pour leur composante selon $y$.
Les parois perpendiculaires et parallèles peuvent être choisies conductrices ou
isolantes, impliquant un jeu de conditions aux limites physiques sur les flux
en entrée de gaine. Le domaine peut aussi être choisi périodique dans la 
direction $\mathbf y$ pour représenter certaines géométries, et éventuellement
...
itération et pour chaque interaction, MAGNIS lit les coefficients $k_{\alpha
s}$ dans des tables en fonction de l'énergie des
particules. Les valeurs de ces coefficients pour les électrons sont illustrés
...
Le terme de perte d'énergie par électron, $Q_e=\int_{\mathbf
	v}\frac{m_\alpha}{2}v^2C_\alpha \text{d}^3\mathbf
	v=-en_e\sum_{s}\left(E_sn_sk_{es}+\frac{2m_e}{m_s}n_sk^m_{es}\varepsilon_e\right)$, est lui aussi fortement
...
termes du modèle.} :
\begin{equation*}
en_eQ_e(T_e^{k+1})=en_e\left(Q_e(T_e^{k})+\frac{\partial Q_e}{\partial
