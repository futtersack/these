...
modèle et les modifications sur le code qui ont été entreprises pour lui
permettre décrire le transport dans des conditions caractéristiques des plasmas
...
compte du transport collisionnel avec les neutres et le modèle intègre des
conditions aux limites de type gaine sur le courant et le flux de particule dans
la direction perpendiculaire au champ magnétique.


\section{Transport fortement magnétisé dans la SOL}
\sectionmark{Transport dans le plasma de bord}
La première phase de cette thèse a consisté à utiliser et modifier le
code TOKAM2D. Le but était alors de se familiariser avec les
techniques développées dans lmécanismes du transport magnétisé tout en  par les 
de pouvoir appliquer au problème des sources d'ions. Après une brève description de d'historique de TOKAM2D et des hypothèses du modèle, les équations du modèle sont dérivées dans leur forme adimensionnée
...
la turbulence d'interchange qui domine le transport transverse dans le plasma
de bord des tokamaks, sa modification pour intégrer un forçage de la turbulence
...
densité que l'on mesurait expérimentalement. L'étude du modèle a ainsi permis
d'améliorer la compréhension du mécanisme de l'instabilité
d'interchange, par la mise en évidence du transport turbulent de type
avalanche et le caractère de déclenchement à seuil de l'instabilité.
