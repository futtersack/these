...
\cite{SarazinPhD}. En effectuant un changement de variable $\Omega=\nabla_\perp^2U$,
où $W$ symbolise une vorticité\footnote{La vorticité d'un écoulement est généralement définie comme le rotationnel de sa vitesse. Dans la SOL, le champ de vitesse correspond essentiellement à la dérive ExB, ce qui donne pour un
champ magnétique constant de 1T : 
$\nabla\times\mathbf{v}\sim\nabla\times\mathbf{u}_E=\nabla\times(\mathbf{E}/B\times\mathbf{b})\equiv-\nabla_\perp^2
U=\Omega$ }, \eqref{EqCourant2} se transforme en une équation simplifiée dite de
vorticité:
\begin{equation}\begin{split}
...
\nabla_\perp^2\Omega+
B\puissance{-1}\left[U,\Omega\right]\right)
\end{split}
\end{equation} 
...
\nabla_\perp^2\Omega+
\frac{1}{B}\left[U,\Omega\right]=\frac{ec_sB^2}{m_iL_\para}\left(1-e^{\Lambda-e\Delta
...
$\text{N}$, le potentiel électrostatique $\Phi$ et la vorticité $\text{W}$ par :

\begin{eqnarray}
n = n_{\indice{0}}\text{N} \text{~~~~~~~} U=\frac{\Phi T\indice{0}}{e} \text{~~~~~~~}
\Omega=\frac{\text{W} T\indice{0}}{e\rho_L^2}
\end{eqnarray}
